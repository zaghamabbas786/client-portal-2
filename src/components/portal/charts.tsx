...
export function EquityChart({
  ...
  const { path, area, ticks, xTicks, min, max } = useMemo(() => {
    const pad = { t: 20, r: 20, b: showAxis ? 28 : 12, l: showAxis ? 58 : 12 };
    if (!data || data.length < 2)
      return {
        path: "",
        area: "",
        ticks: [] as { v: number; y: number }[],
    ...
    const ys2 = (v: number) => pad.t + (1 - (v - mn) / (mx - mn)) * ih;
    let p = "",
      a = "";
    data.forEach((d, i) => {
      const x = xs(i),
    ...
    a =
      p +
      `L${xs(data.length - 1).toFixed(2)} ${(pad.t + ih).toFixed(2)} L${xs(0).toFixed(2)} ${(pad.t + ih).toFixed(2)} Z`;
    ...
    >
      <svg width={w} height={height} style={{ overflow: "visible" }}>
        ...
        <path
          ...
          style={{
            filter:
              "drop-shadow(0 0 6px " +
              (up ? "rgba(61,214,140,0.22)" : "rgba(240,106,106,0.22)") +
              ")",
          }}
        />
        {hover && (
      ...
      {hover && data[hover.idx] && (
        ...
        >
          ...
          <div className="h-v">
            $
            {data[hover.idx].eq.toLocaleString("en-US", {
